...
bot.command("start", async (context) => {
  ...
  if (key) {
    context.reply(
      "Write dowm your currency rate.\nFor example: 1USDT = 100EAD"
    );
  } else {
    context.replyWithHTML(
      `Welcome to the limited-time USDT to Dirhams exchange auction! 🚀`
    );
  }
});
...
bot.action(
  ...
  async (context) => {
    const id = context.chat?.id as number;
    ...
    if (context.match[0].match(/I accepted to buy \d+/)) {
      ...
      const idToReply =
        context.match[0].split(" ")[context.match[0].split(" ").length - 1];

      try {
        const resUser = await axios.get(
          `https://p2p-backend-4mva.onrender.com/wallet/${id}`
        );
    ...
    if (context.match[0].match(/I already sent Dirhams to \d+/)) {
      ...
      try {
        bot.telegram.sendMessage(
          idToReply,
          `The partner already sent Dirhams to your bank account.\n\nIf you receive a notification from your account, please click on the button bellow 👇🏽👇🏽`,
          {
            reply_markup: {
              inline_keyboard: [
                [
